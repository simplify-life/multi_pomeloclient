...
const PomeloClient = function () {
    var JS_WS_CLIENT_TYPE = 'js-websocket';
    var JS_WS_CLIENT_VERSION = '0.0.1';
    var WebSocket = require('ws');
    var Protocol = require('pomelo-protocol');
    var protobuf = require('pomelo-protobuf');
    var decodeIO_protobuf = null;
    ...
    var connect = function (params, url, cb) {
        ...
        if (window.protos && protoVersion === 0) {
            var protos = JSON.parse(window.protos);
        ...
        var onerror = function (event) {
            self.pomelo.emit('io-error', event);
            console.error('socket error: ', event);
        };
        var onclose = function (event) {
            self.pomelo.emit('close', event);
            self.pomelo.emit('disconnect', event);
    ...
    var sendMessage = function (reqId, route, msg) {
        ...
        //console.log('buffer',packet.buffer);
        send(packet);
    };
    ...
    var heartbeatTimeoutCb = function () {
        ...
        if (gap > gapThreshold) {
            ...
            self.pomelo.emit('heartbeat timeout');
    ...
    var handshake = function (data) {
        ...
        if (data.code === RES_OLD_CLIENT) {
            self.pomelo.emit('error', 'client version not fullfill');
            return;
        }

        if (data.code !== RES_OK) {
            self.pomelo.emit('error', 'handshake fail');
            return;
        }
    ...
    var onData = function (data) {
        ...
        processMessage(self.pomelo, msg);
    };

    var onKick = function (data) {
        data = JSON.parse(Protocol.strdecode(data));
        self.pomelo.emit('onKick', data);
    };
    ...
    var initData = function (data) {
        ...
        if (protos) {
            ...
            window.protos = JSON.stringify(protos);
